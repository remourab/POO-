...
export class App {
    ...
    Location(bikeId: string, latitude: number, longitude: number, city: string, timestamp:number){
        const newbike = this.bikes.find(bike => bike.id === bikeId)
        if (!bike) {
            throw new Error('Bike not found.')
        }
        ...
        bike.Location.timestamb = timestamp;
    }
}
